...
peru_LT = 
  LifeTables::lt.mx(peru_mx$mx, age = peru$age )$lt %>% 
  as_tibble() %>%
  mutate(age = 
           paste0(Age, "-", Age + 4) %>% 
           inset(c(1, 2, length(Age)), c("0", "1-4", "100+")), .before=1)
...
peru_0_to_5 =peru_LT %>%
  filter(Age < 5) %>%
  summarise(age = "0-4", npx = prod(npx), nqx = 1 - prod(npx))
...
peru_pop <- popF %>% filter(name == "Peru") %>% select(age, pop = `2015`)
...
peru_Fx <- percentASFR %>%
  ...
  left_join(peru_LT, by = "age") %>%
  mutate(
    f_tilde =
      asfr_2_f_tilde(asfr, peru_srb, lag(sx), lag(pop), pop, thailand_q0)
  ) %>%
